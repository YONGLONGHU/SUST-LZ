		...
		methods: {
			...
			filterData() {
				...
				this.filteredData = this.allData.filter(item => {
					// 判断报警名称（模糊匹配）
					const matchAlarmName = !this.filters.alarmName || item.AlarmName.includes(this.filters
						.alarmName);

					// 判断设备名称（模糊匹配）
					const matchDeviceName = !this.filters.deviceName || item.DeviceId.includes(this.filters
						.deviceName);

					// 判断产品类型（精确匹配）
					const matchProductType = !this.filters.productType || item.ProductClass === this.filters
						.productType;

					// 判断时间区间（起始时间和结束时间都需要满足）
					const matchTimeRange = (!this.filters.timeRange?.start || new Date(item.AlarmTime) >= new Date(
							this.filters.timeRange.start)) &&
						(!this.filters.timeRange?.end || new Date(item.AlarmTime) <= new Date(this.filters
							.timeRange.end));

					// 判断报警分类（精确匹配）
					const matchAlertCategory = !this.filters.alertCategory || item.AlarmClass === this.filters
						.alertCategory;

					// 判断报警等级（精确匹配，忽略大小写和空格差异）
					const matchAlertLevel = !this.filters.alertLevel || item.AlarmLevel.trim().toLowerCase() ===
						this.filters.alertLevel.trim().toLowerCase();
					// 判断是否确认（布尔值匹配）
					const matchIsConfirmed = !this.filters.isConfirmed || item.Confirm === this.filters
						.isConfirmed;
					// 返回所有条件均满足的数据
